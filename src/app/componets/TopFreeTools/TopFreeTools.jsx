        ...
        <>
            <div className='pt-10 px-10'>
                ...
                <OwlCarousel className='owl-theme AI_Tools' id='light' nav={false} margin={23} dots={false} responsive={{ 0: { items: 1 }, 500: { items: 2 }, 900: { items: 3 }, 1100: { items: 4 }, 1299: { items: 5 } }}>
                    {
                        cards.map((card) => (
                            <div key={card.id} className='rounded-md px-4 py-3  hover:dark:bg-dark-grey dark:bg-dark-black dark:text-white w-full bg-white hover:bg-light-gray  border-1  dark:border-card-border '>
                                <div className='flex justify-between items-center  h-14'>
                                    <div className='flex'>
                                        <div className='w-10 h-10 rounded-md  me-3 dark:border-card-border  border-1 '>
                                            <Image src={card.icon} className='rounded-md dark:border-slate-50  border-dark-black h-full w-full object-contain' alt={card.title} width={40} height={40} />
                                        </div>
                                        <div className='font-semibold'>
                                            <p className='text-sm'>{card.title}</p>
                                            <i className='flex text-xs py-1'>
                                                {
                                                    card.star.map((star, index) => (
                                                        <IoIosStar key={index} style={{ paddingRight: '2px' }} color={star} />
                                                    ))
                                                }
                                            </i>
                                        </div>
